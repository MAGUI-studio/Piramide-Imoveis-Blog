...
export function CitiesExplorer({ cities = [] }: CitiesExplorerProps) {
  ...
  const filteredCities = useMemo(() => {
    return cities.filter((city) => {
      if (searchTerm && searchTerm.trim()) {
        ...
        if (!matchName && !matchState && !matchDesc) return false;
      }
      return true;
    });
  }, [cities, searchTerm]);
  ...
  const totalPages = Math.max(1, Math.ceil(sortedCities.length / currentLimit));
  const safeCurrentPage = Math.min(Math.max(1, currentPage || 1), totalPages);

  const paginatedCities = useMemo(() => {
    const start = (safeCurrentPage - 1) * currentLimit;
    return sortedCities.slice(start, start + currentLimit);
  }, [sortedCities, safeCurrentPage, currentLimit]);
      ...
      <div className="space-y-5">
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder="Pesquisar por nome da cidade, estado ou região..."
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <select
                ...
                className="w-full py-2.5 pl-3.5 pr-8 bg-white dark:bg-zinc-950 border border-zinc-300 dark:border-white/15 text-xs sm:text-sm font-mono text-foreground focus:outline-none focus:border-primary transition-colors appearance-none cursor-pointer"
              >
                <option value={6}>6 cidades</option>
                <option value={12}>12 cidades</option>
                <option value={24}>24 cidades</option>
              </select>
              <Icon
                icon="ph:caret-down-bold"
                className="absolute right-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground pointer-events-none"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-1 text-xs font-mono">
          <div className="text-muted-foreground">
            ...
            <strong className="text-foreground">
              {sortedCities.length > 0 ? (safeCurrentPage - 1) * currentLimit + 1 : 0} -{" "}
              {Math.min(safeCurrentPage * currentLimit, sortedCities.length)}
            </strong>{" "}
            de <strong className="text-foreground">{sortedCities.length}</strong> cidades encontradas
          </div>
      ...
      {paginatedCities.length > 0 ? (
        ...
        <div className="w-full py-16 px-6 text-center border border-dashed border-zinc-300 dark:border-white/15 space-y-4">
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            Não encontramos cidades com os termos digitados. Tente buscar por outros nomes.
          </p>
          <button
            type="button"
            onClick={handleClearFilters}
