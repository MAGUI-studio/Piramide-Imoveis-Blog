...
export function CategoriesExplorer({ categories = [] }: CategoriesExplorerProps) {
  ...
  const totalPages = Math.max(1, Math.ceil(sortedCategories.length / currentLimit));
  const safeCurrentPage = Math.min(Math.max(1, currentPage || 1), totalPages);

  const paginatedCategories = useMemo(() => {
    const start = (safeCurrentPage - 1) * currentLimit;
    return sortedCategories.slice(start, start + currentLimit);
  }, [sortedCategories, safeCurrentPage, currentLimit]);
      ...
      <div className="space-y-5">
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder="Pesquisar por nome da categoria, temas ou descrição..."
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              >
                ...
                <option value={12}>12 categorias</option>
                <option value={24}>24 categorias</option>
              </select>
              <Icon
                icon="ph:caret-down-bold"
                className="absolute right-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground pointer-events-none"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-1 text-xs font-mono">
          <div className="text-muted-foreground">
            ...
            <strong className="text-foreground">
              {sortedCategories.length > 0 ? (safeCurrentPage - 1) * currentLimit + 1 : 0} -{" "}
              {Math.min(safeCurrentPage * currentLimit, sortedCategories.length)}
            </strong>{" "}
            de <strong className="text-foreground">{sortedCategories.length}</strong> categorias encontradas
          </div>
      ...
      {paginatedCategories.length > 0 ? (
        ...
        <div className="w-full py-16 px-6 text-center border border-dashed border-zinc-300 dark:border-white/15 space-y-4">
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            Não encontramos categorias com os termos digitados. Tente buscar por outros nomes.
          </p>
          <button
            type="button"
            onClick={handleClearFilters}
