...
export function CityCard({ city, className = "" }: CityCardProps) {
  if (!city.slug?.current) return null;

  const imageUrl = city.image
    ? urlForImage(city.image)?.width(700).height(500).fit("crop").url()
    : null;

  return (
    <Link
      href={`/cidade/${city.slug.current}`}
      ...
      <div className="relative z-10 space-y-2 text-white pointer-events-none w-full">
        <div>
          <span className="px-2.5 py-1 bg-black/50 backdrop-blur-md font-mono text-[10px] font-bold uppercase tracking-widest text-white shadow-xs inline-block transition-all border border-white/10">
            {city.postCount || 0} {city.postCount === 1 ? "Artigo" : "Artigos"}
          </span>
        </div>
