...
import type { LaunchItem } from "@/src/types/sanity";

export interface LaunchesCarouselProps {
  launches?: LaunchItem[];
}
...
export function LaunchesCarousel({ launches = [] }: LaunchesCarouselProps) {
  const items =
    launches.length > 0
      ? launches
          .map((item) => {
            const imageUrl = item.image
              ? urlForImage(item.image)?.width(800).height(800).url()
              : null;
            return {
              id: item._id,
              title: item.title,
              ...
              image: imageUrl || "",
              href: item.href || "https://www.piramideimoveissjc.com.br/",
            };
          })
          .filter((item) => Boolean(item.image))
      : [];

  const displayItems = items.length > 0 ? items : fallbackLaunches;
