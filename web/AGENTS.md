...
- **Utility CSS/class Merger**: Import `cn` from `@/src/lib/utils/utils` (or config path).
- **Design Tokens Config**: Import design tokens from `@/src/lib/utils/design-system`.
...
- **`src/lib/utils/docs/[component_name].md`**

Whenever you are working with an installed component, you MUST read its specific markdown file (e.g. `src/lib/utils/docs/button.md` or `src/lib/utils/docs/carousel.md`) to check the exact props, types, defaults, and code examples before writing or refactoring code.
